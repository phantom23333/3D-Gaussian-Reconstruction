...
torch::Tensor py_triangulate(const torch::Tensor& points) {
    ...
    auto cells_out = torch::empty({ static_cast<long>(cells.size()), 4 }, torch::dtype(torch::kInt32).device(torch::kCPU));
    memcpy(
        cells_out.data_ptr(),
...
PYBIND11_MODULE(tetranerf_cpp_extension, m) {
    m.def("triangulate", &py_triangulate, "Triangulate points",
        py::arg("points"));
}
