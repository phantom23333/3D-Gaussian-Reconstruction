## This script is used to extract the mesh from the trained model
## The mesh is extracted using the marching tetrahedra algorithm
## gltf2.0 is implemented and is fully functional
# Inspired by the GOF implementation
# Written By Yu Ji
import torch
...
def marching_tetrahedra_with_binary_search(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, filter_mesh : bool, texture_mesh : bool):
    ...
    for step in range(n_binary_steps):
        ...
        mesh.export(os.path.join(render_path, f"mesh_binary_search_{step}.gltf"))
